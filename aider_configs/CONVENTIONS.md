...
When writing tests, make sure that you ONLY use pytest or pytest plugins. DO NOT use the unittest module under any circumstances. All tests should have typing annotations as well. All tests should be in ./tests. Be sure to create all necessary files and folders. If you are creating files inside of ./tests or ./src/sandbox_agent, be sure to make a __init__.py file if one does not exist. Make sure tests cover all parts of the codebase and account for various edge cases.
...
Avoid using `from unittest.mock` for mocking in tests. Instead, use the `MockerFixture` provided by the `pytest-mock` plugin.
