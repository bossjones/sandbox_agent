...
from sandbox_agent.ai.callback import LoggingCallbackHandler
...
def load_stuff_prompts(prompts: dict | None) -> dict:
    ...
    if prompts.get("initial_prompt"):
        return {"prompt": load_prompt_from_config(prompts.get("initial_prompt"))}
...
def get_summarize_llm() -> BaseChatModel:
    ...
    """
    model = aiosettings.summarize_llm.copy()  # pylint: disable=no-member
    model["callbacks"] = [LoggingCallbackHandler()]

    return ChatModelFactory.create(model)


def summarize(
    text: str, chain_type: str | None = None, initial_prompt: dict | None = None, iteration_prompt: dict | None = None
) -> str:
    ...
    summarize_chain_map = {"stuff": load_stuff_prompts, "map_reduce": load_map_prompts, "refine": load_refine_prompts}
    settings = aiosettings
    chain_type = chain_type or settings.summ_default_chain
    if chain_type not in summarize_chain_map:
        raise ValueError(
            f"Got unsupported chain type: {chain_type}. " f"Should be one of {list(summarize_chain_map.keys())}"
        )
    ...
    prompts_args = summarize_chain_map[chain_type](
        {"initial_prompt": initial_prompt, "iteration_prompt": iteration_prompt}
    )

    chain = load_summarize_chain(**kwargs, **prompts_args)
    text_splitter = TokenTextSplitter(
        chunk_size=settings.summ_token_splitter, chunk_overlap=settings.summ_token_overlap
    )
