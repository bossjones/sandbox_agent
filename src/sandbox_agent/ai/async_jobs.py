...
from sqlalchemy.dialects.postgresql import JSON, SMALLINT, TIMESTAMP
from sqlalchemy.orm import Session

from sandbox_agent.ai.connection import db_connection
from sandbox_agent.ai.services import BaseService
...
def complete_job(
    ...
    if not job_store:
        logger.error("Job %s not found", uuid)
        return
    now = datetime.now()
    if job_store.expires and job_store.expires < now:
        logger.warning("Job %s is expired at %s", uuid, job_store.expires)
        return
    ...
    try:
        ...
        msg = exc.args[0]
        logger.error("Error completing job - %s", msg)
        result = {"error": msg}
        try:
            save_job_result(job_store=job_store, result=result, error=True)
        except Exception as ex2:
            logger.error("Error saving job error - %s", ex2.args[0])
...
def run_job_service(
    ...
    try:
        ...
        logger.error("Error in job service %s: %s", job_store.service, msg)
        result = {"error": msg}
...
def lock_job_service(job_store: AsyncJobsStore) -> None:
    """Lock job service"""
...
def is_job_available(job_store: AsyncJobsStore) -> bool:
