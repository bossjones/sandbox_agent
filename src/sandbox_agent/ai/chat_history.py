...
from sandbox_agent.ai.connection import db_connection
...
from sandbox_agent.utils.uuid import is_valid_uuid
...
class ChatHistoryStore(BaseModel):
    ...
    created = sqlalchemy.Column(
        sqlalchemy.DateTime(timezone=False), nullable=False, server_default=sqlalchemy.sql.func.now()
    )
...
def history(chat_history: list, session: str = None):
    ...
    if len(chat_history) > 0:
        return [(x["query"], x["answer"]) for x in chat_history]  # type: ignore

    if not is_valid_uuid(session) or aiosettings.qa_no_chat_history:
        return []
...
def is_related(chat_history: list, question: str, embeddings: dict | None = None):
    ...
    embeddings_engine = EmbeddingModelFactory.create()
    q_e = embeddings_engine.embed_query(question)
    h_e = embeddings_engine.embed_query("".join([sentence for tuple in chat_history for sentence in tuple]))
    sim = dot_product(q_e, h_e)
    logger.info("similarity: %s", sim)
    threshold = aiosettings.qa_followup_sim_threshold
...
def get_history(filter: ChatHistoryFilter) -> dict:  # pylint: disable=redefined-builtin
    ...
    with Session(db_connection()) as session:
        ...
        result = query_data_pagination(query=query, page=filter.page, page_size=filter.page_size)
        return result
...
def get_history_sessions_query(
    ...
    subquery = (
        ...
        .group_by(ChatHistoryStore.session_id, CollectionStore.name)
    ).subquery()
