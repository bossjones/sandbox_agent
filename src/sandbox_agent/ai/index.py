...
from langchain_community.vectorstores.pgembedding import CollectionStore, EmbeddingStore
...
from sandbox_agent.ai import connection, load_file
from sandbox_agent.ai.collections import single_collection_by_name
from sandbox_agent.ai.embedding_models import EmbeddingModelFactory
from sandbox_agent.aio_settings import aiosettings
from sandbox_agent.types import load_type
from sandbox_agent.utils.json_api import query_data_pagination


# from sandbox_agent.ai.models import load_embeddings
...
def split_document(
    document: Document,
    collection_meta: dict = {},  # type: ignore
) -> list[Document]:
...
def create_splitter(collection_meta: dict) -> TextSplitter:
    ...
    custom_splitter = collection_meta.get("text_splitter", settings.text_splitter.copy())
    chunk_size = int(collection_meta.get("chunk_size", settings.text_chunk_size))
    chunk_overlap = int(collection_meta.get("chunk_overlap", settings.text_chunk_overlap))

    if not custom_splitter:
        return NLTKTextSplitter(separator="\n", chunk_size=chunk_size, chunk_overlap=chunk_overlap)
...
def metadata_filters(filter: dict[str, str] = {}):  # type: ignore
    """
...
def collection_documents(
    collection_id: str,
    filter: dict[str, str] = {},  # type: ignore
    page: int = 1,
    page_size: int = 50,
):
    """Read document `document_id` from collection index"""
    ...
    with Session(connection.db_connection()) as session:
        query = session.query(EmbeddingStore.document, EmbeddingStore.cmetadata, EmbeddingStore.custom_id)
...
def update_collection_link(collection: CollectionStore, document_id: str, document_medatata: dict) -> bool:
    ...
    if document_has_changed(document=document, collection_id=collection.uuid, document_id=document_id):
        remove_document(collection_id=collection.uuid, document_id=document_id)
        document.metadata.pop("http_error", None)
        add_document(document=document, collection_name=collection.name, document_id=document_id)
        return True
