...
from sandbox_agent.backend.cache import goobredis_session


if TYPE_CHECKING:
    from sandbox_agent.backend.cache.goobredis_session import RedisSessionManagerUtility
...
async def test_redis_session_manager_utility():
    ...
    assert (
        str(type(manager._driver))
        == "<class 'sandbox_agent.backend.cache.goobredis.GoobRedisClient'>"
    )
