...
from sandbox_agent.clients.http_client import HttpClient
...
def test_post_throws(mocker):
    ...
    _mock_client_post = mocker.patch(
        "sandbox_agent.clients.http_client.HttpClient.post"
    )
    _mock_client_post.side_effect = [
...
def test_get_throws(mocker: Mocker):
    ...
    mocked_response.text.return_value = err_str
    mocked_response.body = err_str
    _mock_client_post = mocker.patch("sandbox_agent.clients.http_client.HttpClient.get")
    _mock_client_post.side_effect = [
        RequestException("Failed to connect"),
        mocked_response,
    ]
