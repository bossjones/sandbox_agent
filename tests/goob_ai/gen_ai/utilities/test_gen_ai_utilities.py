...
from sandbox_agent.gen_ai.utilities import (
    add_chunk_id,
    add_item_checksum,
...
def test_get_chunks_to_delete() -> None:
    ...
    chunks_prev = [
        Document(
            page_content="Old1",
            metadata={"item_id": "1", "last_seen_at": int(now - 86400 * 2)},
        ),
        Document(
            page_content="Old2",
            metadata={"item_id": "2", "last_seen_at": int(now - 86400 * 1)},
        ),
    ]
...
def test_get_chunks_to_update() -> None:
    ...
    chunks_prev = [
        ...
        Document(
            page_content="Unchanged", metadata={"item_id": "2", "checksum": "def"}
        ),
    ]
    chunks_current = [
...
def test_add_item_checksum() -> None:
    ...
    items = [
        Document(page_content="Test", metadata={"key1": "value1", "key2": "value2"})
    ]
