...
from sandbox_agent.services.chroma_service import (
    ChromaService,
...
def test_calculate_chunk_ids():
    ...
    chunks = [
        Document(
            page_content="Chunk 1", metadata={"source": "data/file1.pdf", "page": 1}
        ),
        Document(
            page_content="Chunk 2", metadata={"source": "data/file1.pdf", "page": 1}
        ),
        Document(
            page_content="Chunk 3", metadata={"source": "data/file1.pdf", "page": 2}
        ),
        Document(
            page_content="Chunk 4", metadata={"source": "data/file2.pdf", "page": 1}
        ),
    ]
...
def test_add_collection(mocker: MockerFixture) -> None:
    ...
    result = ChromaService.add_collection(
...
def test_custom_openai_embeddings_call(
    mocker: MockerFixture, custom_embeddings: CustomOpenAIEmbeddings
) -> None:
    """
    ...
    mocker.patch.object(
        CustomOpenAIEmbeddings, "_embed_documents", return_value=mock_embeddings
    )
...
def mock_pdf_file(tmp_path: Path) -> Path:
    ...
    shutil.copy(
        "src/sandbox_agent/data/chroma/documents/rich-readthedocs-io-en-latest.pdf",
        test_pdf_path,
    )
...
def mock_txt_file(tmp_path: Path) -> Path:
    ...
    shutil.copy(
        "src/sandbox_agent/data/chroma/documents/state_of_the_union.txt", test_txt_path
    )
...
def test_split_text(mocker: MockerFixture) -> None:
    ...
    from sandbox_agent.services.chroma_service import split_text
    from langchain.schema import Document
    ...
    mock_text_splitter: MagicMock | AsyncMock | NonCallableMagicMock = mocker.patch(
        "sandbox_agent.services.chroma_service.RecursiveCharacterTextSplitter"
    )
    ...
    mock_text_splitter.return_value.split_documents.assert_called_once_with(
        mock_documents
    )
    ...
    mock_text_splitter = mocker.patch(
        "sandbox_agent.services.chroma_service.RecursiveCharacterTextSplitter"
    )
    mock_text_splitter.return_value.split_documents.return_value = mock_chunks
...
def test_chroma_service_e2e(mocker: MockerFixture, mock_txt_file: Path) -> None:
    ...
    db = Chroma.from_documents(
        docs, embedding_function, collection_name=test_collection_name, client=client
    )
...
def test_chroma_service_e2e_add_to_chroma(
    ...
    db = ChromaService.add_to_chroma(
        path_to_document=f"{mock_txt_file}",
...
@pytest.mark.integration()
@pytest.mark.e2e()
def test_chroma_service_e2e_add_to_chroma_disallowed_special(
    mocker: MockerFixture, mock_txt_file: Path
) -> None:
    client = ChromaService.client
    test_collection_name = "test_chroma_service_e2e_add_to_chroma_disallowed_special"

    embeddings = get_rag_embedding_function(
        filename=f"{mock_txt_file}", disallowed_special=()
    )
...
def test_get_rag_embedding_function_txt(mock_txt_file: Path) -> None:
    ...
    assert "langchain_community.embeddings.huggingface.HuggingFaceEmbeddings" in str(
...
def test_get_rag_embedding_function_github_io_url(mock_github_io_url: str) -> None:
    ...
    assert "langchain_openai.embeddings.base.OpenAIEmbeddings" in str(
        type(embedding_function)
    )
...
@pytest.mark.skip(
    reason="This is a work in progress and it is currently expected to fail"
)
@pytest.mark.flaky()
@pytest.mark.services()
...
@pytest.mark.vcr(
    allow_playback_repeats=True, match_on=["request_matcher"], ignore_localhost=False
)
def test_search_db_returns_relevant_documents(
    caplog: LogCaptureFixture, capsys: CaptureFixture, vcr
):
    """
    ...
    from sandbox_agent.services.chroma_service import _await_server
...
def test_generate_context_text() -> None:
    ...
    """
    from sandbox_agent.services.chroma_service import generate_context_text
    from langchain.schema import Document
...
def test_generate_prompt(capsys: CaptureFixture, caplog: LogCaptureFixture) -> None:
    ...
    """
    from sandbox_agent.services.chroma_service import generate_prompt
...
async def test_generate_document_hashes():
    docs = [
        Document(
            ...
            metadata={"source": "test_source", "id": "1"},
        ),
        Document(
            page_content="Test document 2",
            metadata={"source": "test_source", "id": "2"},
        ),
    ]
...
#     mocker.patch("sandbox_agent.services.chroma_service.rm_chroma_db", return_value=None)
#     mocker.patch("sandbox_agent.services.chroma_service.Chroma.from_documents", return_value=None)
...
def test_add_to_chroma(
    ...
    from sandbox_agent.services.chroma_service import _await_server, load_documents
    ...
    mock_get_rag_splitter = mocker.patch(
        "sandbox_agent.services.chroma_service.get_rag_splitter"
    )
    mock_get_chroma_db = mocker.patch(
        "sandbox_agent.services.chroma_service.get_chroma_db"
    )
    mock_get_rag_loader = mocker.patch(
        "sandbox_agent.services.chroma_service.get_rag_loader"
    )
...
def test_add_or_update_documents_new_documents(
    ...
    mock_get_rag_embedding_function = mocker.patch(
        "sandbox_agent.services.chroma_service.get_rag_embedding_function"
    )
...
def test_add_or_update_documents_existing_documents(
    ...
    assert mock_chroma_db.add_documents.call_args.args == (
        [
            Document(
                metadata={"start_index": 0, "id": "None:None:0"},
                page_content="Test document",
            ),
            Document(
                metadata={"start_index": 0, "id": "None:None:1"},
                page_content="Test document",
            ),
            Document(
                metadata={"start_index": 0, "id": "None:None:2"},
                page_content="Test document",
            ),
            Document(
                metadata={"start_index": 0, "id": "None:None:3"},
                page_content="Test document",
            ),
        ],
    )
