...
from sandbox_agent.services.pgvector_service import PgvectorService
...
@pytest.mark.services()
@pytest.mark.pgvectoronly()
async def test_custom_similarity_search_with_scores(
    pgvector_service: PgvectorService, mocker
) -> None:
    ...
    mocker.patch(
        "sandbox_agent.services.pgvector_service.Session", return_value=mock_session
    )
    results = pgvector_service.custom_similarity_search_with_scores(query, k)

    assert len(results) == 3
    assert all(
        isinstance(doc, Document) and isinstance(score, float) for doc, score in results
    )
...
@pytest.mark.services()
@pytest.mark.pgvectoronly()
async def test_update_pgvector_collection(
    pgvector_service: PgvectorService, mocker
) -> None:
    ...
    mock_pgvector = mocker.patch("sandbox_agent.services.pgvector_service.PGVector")
...
async def test_get_collections(pgvector_service: PgvectorService, mocker) -> None:
    ...
    mock_text = mocker.patch("sandbox_agent.services.pgvector_service.text")
...
async def test_update_collection(pgvector_service: PgvectorService, mocker) -> None:
    ...
    mocker.patch.object(
        pgvector_service, "get_collections", return_value=["existing_collection"]
    )
...
def test_integration_update_pgvector_collection(
    pgvector_service: PgvectorService, vcr: Any
) -> None:
...
def test_integration_get_collections(
    pgvector_service: PgvectorService, vcr: Any
) -> None:
...
def test_integration_update_collection(
    pgvector_service: PgvectorService, vcr: Any
) -> None:
...
def test_integration_delete_collection(
    pgvector_service: PgvectorService, vcr: Any
) -> None:
...
def test_integration_create_collection(
    pgvector_service: PgvectorService, vcr: Any
) -> None:
    ...
    collection_id, doc_ids = pgvector_service.create_collection(
        collection_name, documents, video_metadata
    )
...
@pytest.mark.flaky()
async def test_integration_get_collection_id_by_name(
    pgvector_service: PgvectorService, mocker: MockerFixture
) -> None:
...
@pytest.mark.flaky()
async def test_integration_get_collection_metadata(
    pgvector_service: PgvectorService, mocker: MockerFixture
) -> None:
    ...
    store = pgvector_service.get_vector_store(
        collection_name, metadatas=expected_metadata
    )
    store.__post_init__()

    documents = [Document(page_content="Test document for metadata")]

    real_collection_id, doc_ids = pgvector_service.create_collection(
        collection_name, documents, expected_metadata
    )
...
@pytest.mark.flaky()
async def test_integration_update_collection_metadata(
    pgvector_service: PgvectorService, mocker: MockerFixture
) -> None:
    ...
    updated_metadata = pgvector_service.update_collection_metadata(
        collection_id, new_metadata
    )
...
@pytest.mark.flaky()
async def test_integration_list_collections(
    pgvector_service: PgvectorService, mocker: MockerFixture
) -> None:
...
@pytest.mark.flaky()
async def test_integration_get_by_ids(
    pgvector_service: PgvectorService, mocker: MockerFixture
) -> None:
...
@pytest.mark.flaky()
async def test_integration_get_all_by_collection_id(
    pgvector_service: PgvectorService, mocker: MockerFixture
) -> None:
