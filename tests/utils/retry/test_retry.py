...
from sandbox_agent.utils import retry
...
def test__base_parameters(mocker) -> None:
    ...
    spy_tenacity_before_sleep_log: MockType = mocker.spy(
        retry.tenacity, "before_sleep_log"
    )
    spy_tenacity_stop_after_attempt: MockType = mocker.spy(
        retry.tenacity, "stop_after_attempt"
    )
...
def test_linear_backoff_parameters(mocker) -> None:
    ...
    spy_linear_backoff_parameters: MockType = mocker.spy(
        retry, "linear_backoff_parameters"
    )
...
def test_exponential_backoff_parameters(mocker) -> None:
    ...
    spy_exponential_backoff_parameters = mocker.spy(
