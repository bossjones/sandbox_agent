...
from sandbox_agent.utils import base
...
def test_check_whoami(mocker: MockerFixture) -> None:
    ...
    mocker.patch("sandbox_agent.utils.base.get_sys_module", return_value=mock_sys)
...
def test_fltr(
    node: Union[dict, list], whitelist: list[str], expected: Union[dict, list, None]
) -> None:
