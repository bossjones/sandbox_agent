...
from sandbox_agent.services.chroma_service import DATA_PATH
...
# DATA_PATH = os.path.join(HERE, "..", "..", "src", "sandbox_agent", "data", "chroma", "documents")
# CHROMA_PATH = os.path.join(HERE, "..", "..","src", "sandbox_agent", "data", "chroma", "vectorstorage")

# ~/dev/bossjones/sandbox_agent/tests/utils/../../src/sandbox_agent/data/chroma/documents
...
def test_sort_dir_by_mtime(mocker):
    mocker.patch(
        ...
        return_value=[pathlib.Path("file1"), pathlib.Path("file2")],
    )
...
def test_get_all_media_files_to_upload(mocker):
    mock_tree = mocker.patch(
        "sandbox_agent.utils.file_functions.tree", return_value=["file1", "file2"]
    )
    mock_filter_media = mocker.patch(
        "sandbox_agent.utils.file_functions.filter_media", return_value=["file1"]
    )
...
def test_filter_media(mocker):
    mock_filter_images = mocker.patch(
        "sandbox_agent.utils.file_functions.filter_images", return_value=["file1.png"]
    )
    mock_filter_videos = mocker.patch(
        "sandbox_agent.utils.file_functions.filter_videos", return_value=["file2.mp4"]
    )
...
#     mock_filter_pdfs = mocker.patch("sandbox_agent.utils.file_functions.filter_pdfs", return_value=["file1.pdf"])
...
def test_get_dataframe_from_csv(mocker):
    mock_read_csv = mocker.patch(
        "pandas.read_csv", return_value=pd.DataFrame({"col1": [1], "col2": [2]})
    )
    result = get_dataframe_from_csv(
        "/Users/malcolm/dev/bossjones/sandbox_agent/test.csv"
    )
...
def test_check_file_size(mocker):
    mock_stat = mocker.patch(
        "pathlib.Path.stat", return_value=mocker.Mock(st_size=1024)
    )
...
def test_expand_path_str(mocker):
    mock_expanduser = mocker.patch(
        "pathlib.Path.expanduser", return_value=pathlib.Path("/home/user")
    )
...
def test_fix_path(mocker):
    mock_is_file = mocker.patch(
        "sandbox_agent.utils.file_functions.is_file", return_value=True
    )
    mock_tilda = mocker.patch(
        "sandbox_agent.utils.file_functions.tilda", return_value="/home/user/file"
    )
    result = fix_path("~")
