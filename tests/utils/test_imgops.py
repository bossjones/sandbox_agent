...
from sandbox_agent.utils.imgops import (
    auto_split_upscale,
    bgr_to_rgb,
...
def test_auto_split_upscale_no_split(test_image):
    ...
    scale = 2
    overlap = 0
    upscaled_image, depth = auto_split_upscale(
        test_image, dummy_upscale_function, scale, overlap
    )
...
#     mocker.patch("src.sandbox_agent.utils.imgops.dummy_upscale_function", side_effect=RuntimeError("Out of memory"))
...
def test_bgr_to_rgb(test_image):
    ...
    test_image_tensor = (
        torch.from_numpy(test_image).permute(2, 0, 1).float() / 255.0
    ...
    rgb_image = (
        rgb_image_tensor.permute(1, 2, 0).numpy() * 255.0
    )  # CHW to HWC and denormalize
...
def test_convert_tensor_to_pil_image2(test_image):
    ...
    assert chw_image_tensor.shape == (
        test_image_hwc.shape[2],
        test_image_hwc.shape[0],
        test_image_hwc.shape[1],
    )
...
def test_convert_pil_image_to_rgb_channels(test_image, mocker):
    ...
    from sandbox_agent.utils.imgops import convert_pil_image_to_rgb_channels

    # Apply convert_pil_image_to_rgb_channels
    converted_image = convert_pil_image_to_rgb_channels(
...
def test_convert_pil_image_to_torch_tensor(test_image):
    ...
    from sandbox_agent.utils.imgops import convert_pil_image_to_torch_tensor
...
#     from sandbox_agent.utils.imgops import get_pixel_rgb
#     from PIL import Image
...
async def test_setup_model(mocker):
    ...
    from sandbox_agent.utils.imgops import setup_model

    # Mock the load_model function
    mock_load_model = mocker.patch(
        "sandbox_agent.utils.imgops.load_model", return_value=mocker.Mock()
    )
    ...
    mocker.patch(
        "sandbox_agent.utils.imgops.convert_pil_image_to_rgb_channels",
        return_value=mock_image,
    )
    mocker.patch("sandbox_agent.utils.imgops.pred_and_store", return_value=mock_bboxes)
...
async def test_pred_and_store(mocker):
    ...
    from sandbox_agent.utils.imgops import pred_and_store, setup_model

    # from sandbox_agent.utils.imgops import load_model, setup_model
    model = setup_model()
...
async def test_np2tensor():
    ...
    from sandbox_agent.utils.imgops import np2tensor
    ...
    # from sandbox_agent.utils.imgops import handle_resize_one
...
def test_norm(input_tensor, expected_output):
    ...
    from sandbox_agent.utils.imgops import norm
...
async def test_tensor2np(mocker):
    ...
    from sandbox_agent.utils.imgops import tensor2np
...
def test_read_image_to_bgr(mocker):
    ...
    from sandbox_agent.utils.imgops import read_image_to_bgr
...
def test_resize_image_and_bbox(mocker, test_image, return_percent_coords):
    ...
    # mocker.patch("sandbox_agent.utils.imgops.Image.open", return_value=Image.open(test_image_path))
    # mocker.patch("sandbox_agent.utils.imgops.cv2.cvtColor", return_value=np.array(Image.open(test_image_path)))
    # mocker.patch("sandbox_agent.utils.imgops.cv2.imwrite", return_value=True)
    # mocker.patch("sandbox_agent.utils.imgops.file_functions.fix_path", return_value=test_image_path)
...
def test_handle_predict_one(mocker):
    ...
    test_image, test_bboxes = handle_predict_one(
        images_filepath=image_path, model=model
    )
...
def test_handle_autocrop(mocker):
    ...
    cropped_image_paths = handle_autocrop(
        images_filepaths=[image_path], model=model, predict_results=predict_results
    )

    assert len(cropped_image_paths) == 1
    assert (
        "cropped-ObjLocModelV1-screenshot_image_larger00013.PNG"
        in cropped_image_paths[0]
    )
...
def test_handle_autocrop_one(mocker):
    ...
    cropped_image_path = handle_autocrop_one(
        images_filepath=image_path, model=model, predict_results=predict_results
    )

    # assert cropped_image_path == image_path
    assert (
        "cropped-ObjLocModelV1-screenshot_image_larger00013.PNG" in cropped_image_path
    )
...
#     """Test handle_autocrop_one function."""
#     image_path = "tests/fixtures/screenshot_image_larger00013.PNG"
#     mocker.patch("sandbox_agent.utils.imgops.Image.open", return_value=Image.open(image_path))
#     mocker.patch("sandbox_agent.utils.imgops.cv2.cvtColor", return_value=np.array(Image.open(image_path)))
#     mocker.patch("sandbox_agent.utils.imgops.cv2.imwrite", return_value=True)
#     mocker.patch("sandbox_agent.utils.imgops.file_functions.fix_path", return_value=image_path)
...
def test_rgb2hex(r, g, b, expected_hex):
    ...
    from sandbox_agent.utils.imgops import rgb2hex
...
def test_convert_rgb_to_names():
    ...
    from sandbox_agent.utils.imgops import convert_rgb_to_names
...
#     mocker.patch("sandbox_agent.utils.imgops.get_pil_image_channels", return_value=4)
...
def test_rgba_to_bgra(test_image):
    ...
    from sandbox_agent.utils.imgops import rgba_to_bgra

    # Convert the test image to a tensor with an alpha channel
    test_image_tensor = (
        torch.from_numpy(np.dstack((test_image, np.full(test_image.shape[:2], 255))))
        .permute(2, 0, 1)
        .float()
        / 255.0
    )  # HWC to CHW and normalize
    ...
    # Convert back to numpy for comparison
    bgra_image = (
        bgra_image_tensor.permute(1, 2, 0).numpy() * 255.0
    )  # CHW to HWC and denormalize
    ...
    assert np.array_equal(
        bgra_image[:, :, 3], np.full(test_image.shape[:2], 255)
    )  # A channel
...
def test_rgb_to_bgr_async(test_image):
    ...
    # Convert back to numpy for comparison
    bgr_image = (
        bgr_image_tensor.permute(1, 2, 0).numpy() * 255.0
    )  # CHW to HWC and denormalize
